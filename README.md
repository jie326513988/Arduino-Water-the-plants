...
### 请下载最新程序进行更新<br>注意！目前的PCB未加到电池低压保护，电池过放会导致电池寿命下降，请勿让电池电压低于3.4V。
#### 2020-02-25 更新092C  1.按键取消延时的方式消抖，改为对比时间的方式消抖，效果更佳。
#### 2019-12-31 更新091c  1.将水泵超时的警告取消，改成水泵超时后进入休眠 2.优化休眠唤醒后误判断传感器错误 3.设置界面湿度数值前面增加负号。
...
## 烧录说明<br>
        1.使用ArduinoIDE编译，需要下载两次程序才可以使用在setup()找到这段程序看说明下载程序
        EEPROM.put第一次写入去掉注释，第二次以后注释上
        EEPROM.get第一次写入注释上，第二次以后写入去掉注释
        2.使用烧录软件也是要烧录两次hex文件，先烧录1，在烧录2。
        3.必须将arduino pro mini的电源指示LED和LED旁边的限流电阻焊下来，否则电量会很快耗尽，若只焊下LED没焊下电阻就读取电压会烧坏板子！
